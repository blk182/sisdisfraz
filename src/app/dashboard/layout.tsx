    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar { width: ${collapsed ? '72px' : '240px'}; background: #0D0B08; border-right: 1px solid rgba(201,168,76,0.12); display: flex; flex-direction: column; position: fixed; top: 0; left: 0; bottom: 0; z-index: 100; transition: width 0.25s ease; overflow: hidden; }
        .sidebar-header { padding: ${collapsed ? '20px 0' : '24px 20px'}; border-bottom: 1px solid rgba(201,168,76,0.08); display: flex; align-items: center; gap: 12px; justify-content: ${collapsed ? 'center' : 'flex-start'}; }
        .sidebar-logo { font-size: 28px; flex-shrink: 0; }
        .sidebar-brand { overflow: hidden; opacity: ${collapsed ? 0 : 1}; transition: opacity 0.2s; white-space: nowrap; }
        ...
        .nav-label { font-family: 'Cinzel', serif; font-size: 8px; letter-spacing: 2px; text-transform: uppercase; color: rgba(201,168,76,0.25); padding: 8px 20px 4px; display: ${collapsed ? 'none' : 'block'}; }
        .nav-item { display: flex; align-items: center; gap: 12px; padding: ${collapsed ? '12px 0' : '11px 20px'}; justify-content: ${collapsed ? 'center' : 'flex-start'}; color: rgba(232,224,208,0.5); text-decoration: none; font-family: 'Crimson Text', serif; font-size: 15px; transition: all 0.15s; position: relative; cursor: pointer; }
        ...
        .nav-text { white-space: nowrap; overflow: hidden; opacity: ${collapsed ? 0 : 1}; transition: opacity 0.2s; }
        .sidebar-footer { padding: ${collapsed ? '16px 0' : '16px 20px'}; border-top: 1px solid rgba(201,168,76,0.08); }
        .user-card { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; justify-content: ${collapsed ? 'center' : 'flex-start'}; }
        .user-avatar { width: 36px; height: 36px; border-radius: 50%; background: rgba(201,168,76,0.15); border: 1px solid rgba(201,168,76,0.3); display: flex; align-items: center; justify-content: center; font-size: 16px; flex-shrink: 0; }
        .user-info { overflow: hidden; opacity: ${collapsed ? 0 : 1}; transition: opacity 0.2s; white-space: nowrap; }
        .user-name { font-family: 'Crimson Text', serif; font-size: 14px; color: #E8E0D0; overflow: hidden; text-overflow: ellipsis; max-width: 140px; }
        .user-rol { font-family: 'Cinzel', serif; font-size: 8px; letter-spacing: 1px; text-transform: uppercase; color: ${ROL_COLORS[rol]}; }
        .btn-logout { width: 100%; padding: ${collapsed ? '8px 0' : '8px 12px'}; background: rgba(192,57,43,0.08); border: 1px solid rgba(192,57,43,0.2); border-radius: 2px; color: rgba(192,57,43,0.7); font-family: 'Cinzel', serif; font-size: 9px; letter-spacing: 2px; text-transform: uppercase; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 6px; }
        .btn-logout:hover { background: rgba(192,57,43,0.15); color: #E74C3C; border-color: rgba(192,57,43,0.4); }
        .btn-collapse { position: absolute; top: 24px; right: -12px; width: 24px; height: 24px; background: #1C1A14; border: 1px solid rgba(201,168,76,0.2); border-radius: 50%; color: #C9A84C; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 10px; z-index: 101; transition: all 0.2s; }
        .btn-collapse:hover { background: rgba(201,168,76,0.1); }
        .main { margin-left: ${collapsed ? '72px' : '240px'}; flex: 1; transition: margin-left 0.25s ease; min-height: 100vh; background: #0A0A0A; }
      ...
      <div className="layout">
        <aside className="sidebar">
          <button className="btn-collapse" onClick={() => setCollapsed(!collapsed)}>{collapsed ? '›' : '‹'}</button>
