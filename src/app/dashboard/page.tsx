  ...
  return (
    <>
      <style>{`
        ...
        .dash-date { font-family: 'Crimson Text', serif; font-style: italic; font-size: 14px; color: rgba(201,168,76,0.4); text-transform: capitalize; letter-spacing: 1px; margin-bottom: 4px; }
        .dash-title { font-family: 'Cinzel', serif; font-size: 24px; font-weight: 700; color: #E8E0D0; letter-spacing: 1px; }
        .dash-hora { font-family: 'Cinzel', serif; font-size: 13px; color: rgba(201,168,76,0.5); margin-top: 2px; }
        .cards-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 28px; }
        @media (max-width: 900px) { .cards-grid { grid-template-columns: repeat(2, 1fr); } }
        .alert-card { background: #141210; border-radius: 3px; padding: 22px 20px; position: relative; overflow: hidden; transition: transform 0.2s; }
        .alert-card:hover { transform: translateY(-2px); }
        .alert-card-border { position: absolute; top: 0; left: 0; right: 0; height: 2px; }
        .alert-card-glow { position: absolute; top: 0; left: 0; right: 0; bottom: 0; opacity: 0.04; }
        .card-icon { font-size: 28px; margin-bottom: 12px; display: block; }
        .card-value { font-family: 'Cinzel', serif; font-size: 42px; font-weight: 700; line-height: 1; margin-bottom: 6px; }
        .card-label { font-family: 'Crimson Text', serif; font-size: 13px; color: rgba(232,224,208,0.4); letter-spacing: 0.5px; }
        .section-title { font-family: 'Cinzel', serif; font-size: 11px; letter-spacing: 3px; text-transform: uppercase; color: rgba(201,168,76,0.5); margin-bottom: 14px; display: flex; align-items: center; gap: 10px; }
        .section-title::after { content: ''; flex: 1; height: 1px; background: rgba(201,168,76,0.1); }
        .table-wrap { background: #141210; border: 1px solid rgba(201,168,76,0.1); border-radius: 3px; overflow: hidden; }
        .alq-table { width: 100%; border-collapse: collapse; }
        .alq-table th { padding: 12px 16px; text-align: left; font-family: 'Cinzel', serif; font-size: 8px; letter-spacing: 2px; text-transform: uppercase; color: rgba(201,168,76,0.4); background: rgba(201,168,76,0.03); border-bottom: 1px solid rgba(201,168,76,0.08); }
        .alq-table td { padding: 13px 16px; font-family: 'Crimson Text', serif; font-size: 15px; color: #E8E0D0; border-bottom: 1px solid rgba(255,255,255,0.03); }
        .alq-table tr:last-child td { border-bottom: none; }
        .alq-table tr:hover td { background: rgba(201,168,76,0.02); }
        .estado-badge { display: inline-flex; align-items: center; gap: 5px; padding: 3px 8px; border-radius: 2px; font-family: 'Cinzel', serif; font-size: 8px; letter-spacing: 1px; text-transform: uppercase; }
      ...
      <div className="table-wrap">
        {loading ? (
          <div style={{ padding: 24 }}>
            {[1,2,3,4].map(i => <div key={i} className="skeleton" style={{ marginBottom: 12, height: 16, width: `${70 + i * 5}%` }} />)}
          </div>
        ) : alquileres.length === 0 ? (
              ...
              {alquileres.map(a => {
                ...
                return (
                  <tr key={a.id}>
                    ...
                    <td style={{ color: 'rgba(232,224,208,0.6)' }}>{(a.disfraz as any)?.nombre} · {(a.disfraz as any)?.talla}</td>
                    <td>
                    ...
                    <td>
                      <a href={`https://wa.me/${((a.cliente as any)?.whatsapp ?? '').replace('+', '')}`} target="_blank" rel="noreferrer" style={{ color: '#25D366', textDecoration: 'none', fontSize: 13 }}>💬 Contactar</a>
                    </td>
                  </tr>
                )
              })}
